...
def get_irradiance(site_location, date, tilt, surface_azimuth):
    ...
    return pd.DataFrame({'GHI': clearsky['ghi'], 'POA': POA_irradiance['poa_global']}), clearsky, solar_position
...
irradiance_data, cs, solar_position = get_irradiance(site_location, date, tilt=0, surface_azimuth=180)
...
poa_tracker = irradiance.get_total_irradiance(
    ...
    dni=cs['dni'],  # Usamos DNI para la irradiancia directa
    ghi=cs['ghi'],  # Usamos GHI para la irradiancia global
    dhi=cs['dhi'],  # Usamos DHI para la irradiancia difusa
    solar_zenith=solar_position['apparent_zenith'],
    solar_azimuth=solar_position['azimuth']
)
...
full_day_df[['GHI_Measured', 'GHI', 'POA_Tracker']].plot(title='Comparación de GHI Medido, GHI Simulado y POA Modelado (HSAT) para un Día Completo')
plt.ylabel('Irradiancia (W/m^2)')
plt.xlabel('Hora')
plt.legend(['GHI Medido', 'GHI Simulado', 'POA Tracker'])
plt.show()
...
print(f'RMSE (GHI Medido vs. POA Tracker): {rmse:.2f} W/m^2')

# Calcular R²
r2 = r2_score(measured_period['GHI_Measured'], measured_period['POA_Tracker'])
print(f'R² (GHI Medido vs. POA Tracker): {r2:.2f}')

# Calcular RMSE para GHI Simulado vs. GHI Medido
rmse_ghi = mean_squared_error(measured_period['GHI_Measured'], measured_period['GHI'], squared=False)
print(f'RMSE (GHI Medido vs. GHI Simulado): {rmse_ghi:.2f} W/m^2')

# Calcular R² para GHI Simulado vs. GHI Medido
r2_ghi = r2_score(measured_period['GHI_Measured'], measured_period['GHI'])
print(f'R² (GHI Medido vs. GHI Simulado): {r2_ghi:.2f}')
